...
(* legacy pp library, using ppx preprocessor *)
...
let rec pp_term_c fmt = function
  | INT n -> fprintf fmt "%d" n
  | VAR x -> fprintf fmt "%s" x
  | FUN (x, p) -> fprintf fmt "@[<2>fun %s ->@ %a@]" x pp_term_c p
  | APP (p1, p2) -> fprintf fmt "@[<2>(%a %a)@]" pp_term_c p1 pp_term_c p2
  | BOP (p1, op, p2) ->
      fprintf fmt "@[<2>(%a %a %a)@]" pp_term_c p1 pp_op op pp_term_c p2
  | IFZ (p1, p2, p3) ->
      fprintf fmt "@[<2>if %a = 0 then@ %a@ else@ %a@]" pp_term_c p1 pp_term_c
        p2 pp_term_c p3
  | FIX (f, p) -> fprintf fmt "@[<2>fix %s.@ %a@]" f pp_term_c p
  | LET (x, p1, p2) ->
      fprintf fmt "@[<2>let %s = %a in@ %a@]" x pp_term_c p1 pp_term_c p2
  (* --- Pair --- *)
  | PAIR (p1, p2) -> fprintf fmt "@[<2>(%a , %a)@]" pp_term_c p1 pp_term_c p2
  | FST p -> fprintf fmt "@[<2>fst %a@]" pp_term_c p
  | SND p -> fprintf fmt "@[<2>snd %a@]" pp_term_c p
  (* --- List --- *)
  | NIL -> fprintf fmt "nil"
  | CONS (p1, p2) -> fprintf fmt "@[<2>(%a :: %a)@]" pp_term_c p1 pp_term_c p2
  | IFNIL (p1, p2, p3) ->
      fprintf fmt "@[<2>ifnil %a then@ %a@ else@ %a@]" pp_term_c p1 pp_term_c p2
        pp_term_c p3
  | HD p -> fprintf fmt "@[<2>hd %a@]" pp_term_c p
  | TL p -> fprintf fmt "@[<2>tl %a@]" pp_term_c p
  (* --- Tree --- *)
  | LEAF t -> fprintf fmt "@[<2>leaf(%a)@]" pp_term_c t
  | TREE (l, r) -> fprintf fmt "@[<2>tree(%a,@ %a)@]" pp_term_c l pp_term_c r
  | ITEM t -> fprintf fmt "@[<2>item %a@]" pp_term_c t
  | IFLEAF (p1, p2, p3) ->
      fprintf fmt "@[<2>ifleaf %a then@ %a@ else@ %a@]" pp_term_c p1 pp_term_c
        p2 pp_term_c p3
  | LTREE t -> fprintf fmt "@[<2>ltree %a@]" pp_term_c t
  | RTREE t -> fprintf fmt "@[<2>rtree %a@]" pp_term_c t
...
let rec pp_value fmt = function
  ...
  | VFUN (x, p, _) -> fprintf fmt "@[<2>fun %s ->@ %a@]" x pp_term_c p
  | VFIX (f, p, _) -> fprintf fmt "@[<2>fix %s.@ %a@]" f pp_term_c p
  | VFIXFUN (f, x, p, _env) ->
      fprintf fmt "@[<2>fixfun %s ->@ fun %s ->@ %a@]" f x pp_term_c p
  | THUNK (t, _) -> fprintf fmt "@[<2><thunk %a>@]" pp_term_c t
  (* -- Pair -- *)
